...
@app.post("/reset_chain")
async def reset_chain(credentials: HTTPBasicCredentials = Depends(security)):
    # Authenticate admin credentials
    if credentials.username != AUTHORIZED_USER["username"] or credentials.password != AUTHORIZED_USER["password"]:
        raise HTTPException(status_code=401, detail="Unauthorized")

    # Clear the blockchain, keeping only the genesis block
    blockchain.clear()
    create_genesis_block()

    # Broadcast the reset to all nodes
    await broadcast_reset()

    return {"message": "Blockchain reset successfully, syncing with all nodes."}

async def broadcast_reset():
    async with httpx.AsyncClient() as client:
        tasks = [client.post(f"{node}/receive_reset") for node in NODES]
        await asyncio.gather(*tasks, return_exceptions=True)

@app.post("/receive_reset")
async def receive_reset():
    # Reset the blockchain for the node
    blockchain.clear()
    create_genesis_block()

    return {"message": "Blockchain has been reset to genesis block."}

@app.get("/voter_count")
async def voter_count():
    # Use a set to count unique voter IDs
    unique_voters = set()
    
    for block in blockchain:
        if block["data"].get("type") == "voter":
            unique_voters.add(block["data"].get("voter_id"))

    return {"voter_count": len(unique_voters)}
